...
**Reconciling with TD-02's multipart requirement (resolving `AMB-5`):** the completion endpoint's behavior differs by upload path. For a **multipart** upload, the client tracks the `ETag` returned by storage for each uploaded part as it uploads them, then calls `POST /videos/:id/complete-upload` with the ordered list of `{partNumber, eTag}` pairs. The API performs the actual `CompleteMultipartUploadCommand` server-side, using that parts list, via its internal-endpoint admin `S3Client` (TD-09) — this is a metadata-only S3 API call (it assembles the already-uploaded parts into one object) and no video bytes ever touch the API. A successful `CompleteMultipartUploadCommand` response **is** the existence proof for a multipart upload: the command itself fails if the object cannot be assembled, so no separate `HeadObject` check is needed or performed afterward for this path (this supersedes the earlier "`HeadObject`-equivalent check" framing for the multipart case). For the **single-`PUT`** (non-multipart, small file) path, there is no completion command to call against storage — the client simply finished one presigned `PUT`. In that case, the API's `/complete-upload` handler performs a `HeadObject` call against the key computed in TD-03 to confirm the object actually landed before flipping `draft` → `processing` and enqueuing the TD-01 job. Both cases end the same way (status flip + enqueue) but differ in what "verify completion" means: a server-side `CompleteMultipartUploadCommand` for multipart, a `HeadObject` check for single-`PUT`.

**Original filename capture (resolving `MD-3`):** the client supplies an `originalFilename` (or similarly-named field) as part of the `POST` payload that pre-registers the draft video row, and the API persists it on the `videos` row at draft-creation time (see TD-08 for where this persisted value is consumed).

**Ownership and authorization (resolving `MD-5`):** the `videos` table carries a `channelId` column, a FK to `channels.id` — a video belongs to a channel, mirroring the exact FK pattern `Channel` already uses toward `User` (`channel.user_id` as a unique FK column plus a `@JoinColumn`-backed `@ManyToOne`/`@OneToOne` relation). Because `Channel` is a strict 1:1 with `User` (confirmed in `nestjs-project/src/channels/entities/channel.entity.ts`, whose `user_id` column is `unique: true`, and `nestjs-project/src/users/entities/user.entity.ts`, whose `@OneToOne(() => Channel, ...)` back-reference has no list/array shape — each user has exactly one channel, no user has zero or multiple), the upload-initiation endpoint (`POST /videos`, the draft pre-registration call) and `POST /videos/:id/complete-upload` do **not** accept a client-supplied `channelId` (or any channel identifier) in their request payload at all. Instead, both endpoints resolve the target channel automatically from the authenticated request: `req.user` (populated by the JWT guard) identifies the calling user, and the API looks up that user's own channel (the same `user_id`-keyed lookup `ChannelsModule` already performs) to obtain the `channelId` written onto the new `videos` row (at draft creation) or checked against the existing row (at completion). This eliminates the ownership-check/IDOR risk by construction rather than by adding a separate authorization check on top of a client-suppliable id: there is no `channelId` parameter an attacker could substitute for another user's, because the endpoints never read one from the client in the first place. Both endpoints require the existing `JwtAuthGuard` introduced in Fase 02 — no new guard is introduced for this phase.

**Processing→terminal transition (resolving `MD-4`):** the symmetric half of the lifecycle — what the worker (TD-05) writes once it finishes running TD-06's ffprobe/ffmpeg steps — is decided as follows, still within Option A's single `status` enum (no separate history table, no new TD). **Success path:** once ffprobe has reported duration and ffmpeg has generated and uploaded the thumbnail to its TD-03 key (`videos/{videoId}/thumbnail.jpg`), the worker performs one single update on the `videos` row: `status = 'ready'`, `duration` (the ffprobe-reported value), and `thumbnailKey` (the storage key the thumbnail was just written to) — all three fields land together, so no partially-written state is ever visible between "processing" and "ready". **Failure path:** if ffprobe, ffmpeg, or the storage read/write around them (fetching the source object, uploading the thumbnail) throws, the worker's BullMQ job handler catches the exception, writes `status = 'error'` plus `error_message` (a new nullable text column on `videos`, populated only in this state and otherwise `null`) onto the row in the same kind of single update, and then re-throws so the exception surfaces to BullMQ — BullMQ's own configured retry/backoff (TD-01) is what decides whether the job runs again, not any logic in the worker itself. Concretely, the job is configured with **3 attempts and exponential backoff** (BullMQ's `attempts: 3` plus `backoff: { type: 'exponential', delay: <base> }`, the standard pattern TD-01 already points at), rather than leaving the count implicit. Each retry re-invokes the same job handler against the same video row: the handler sets `status = 'processing'` again at the start of the attempt (the same value it already holds from the initial draft→processing write, or from being flipped back after a prior failed attempt) and, at the end of that attempt, lands on `ready` or `error` exactly as above. Once the 3rd attempt also fails, the `error` write from that attempt stands — BullMQ does not schedule any further retry — and the video is left in `error` for good; recovering it requires a manual/administrative re-enqueue, which is out of scope for this phase and deferred to whichever future phase adds that operational tooling.

**Options:**

### Option A: Single `status` enum column on `videos` (`draft` → `processing` → `ready` | `error`)
- One `status` column (backed by a Postgres enum or check constraint) on the `videos` entity. The API sets `draft` at pre-registration (before upload starts, capturing `originalFilename`); the client then calls `POST /videos/:id/complete-upload` once its direct-to-storage upload finishes, the API confirms the object exists in storage (via `CompleteMultipartUploadCommand` for multipart, or a `HeadObject` check for single-`PUT` — see Context above), and only then sets `processing` and enqueues the job; the worker updates the row to `ready` (with extracted metadata) or `error` (with a stored error reason) when the job finishes.
...
**Context:** `docs/diagrams/software-arch.mermaid` explicitly states `Rel(frontend, storage, "Streams", "HTTPS")` — the architecture has already committed to video playback going directly from the client to Object Storage, not proxied through the API. This is a stronger constraint than a mere suggestion: it is drawn as a first-class relation in the C4 diagram, on equal footing with `Rel(frontend, api, "Calls", "REST")`. This decision covers both playback (streaming, with seek support) and the explicit "download the video" capability, since both ultimately need the same mechanism — controlled read access to an object that must support partial/range reads for a scrubbable player. "Shared mechanism" here means one underlying operation (a presigned `GetObjectCommand`), not one literal URL: streaming and download are two distinct call sites against that same operation, differing only in the `ResponseContentDisposition` parameter passed at signing time — the streaming URL is signed with no `ResponseContentDisposition` override (so the browser renders the response inline, letting the `<video>` element play it), while the download URL is signed with `ResponseContentDisposition: 'attachment; filename="<original-filename>"'`, which instructs the browser to save the response to disk instead of playing it. This is a parameter on the presigned GET call itself, not a different download mechanism or a separate endpoint type. **Source of `<original-filename>` (resolving `MD-3`):** this is the `originalFilename` value the client supplied at draft pre-registration time and the API persisted on the video row (see TD-04) — the download-variant presigned GET simply reads that stored column and passes it as the `filename` in the `ResponseContentDisposition` header.

**Status gating (resolving `MD-6`):** the streaming and download endpoints — the ones that issue the presigned GET URLs described above — only succeed when the target video's `status = 'ready'` (TD-04's enum). Any other status (`draft`, `processing`, or `error`) causes the endpoint to reject the request outright rather than issue a presigned URL for an unprocessed or broken file. Concretely: both `draft`/`processing` and `error` return **`404 Not Found`** — a video that isn't `ready` yet is treated as not publicly resolvable at all, the same way an unpublished resource is conventionally hidden rather than exposed as "exists but blocked" with a more specific status code, and a video that permanently failed processing (`error`) is, for this same reason, just as much not a valid public playback/download target as one still `processing`. This keeps a single, consistent "not found" semantics across every non-ready status rather than introducing a distinct 409/423-style status per state, since nothing in this phase's capabilities requires a caller to distinguish "still processing" from "failed" at the HTTP-status level — that distinction, if a future channel-management UI ever needs it (e.g. Fase 04's status-visible dashboard), belongs to that phase's own status-aware surface, not to this phase's public streaming/download endpoints.
...
- Add a `minio` service to `nestjs-project/compose.yaml` (image + published ports, e.g. `9000` for the S3 API and `9001` for the console), mirroring exactly how `mailpit` was added as a new service in Phase 02. The API holds **two** configured endpoint values: `STORAGE_INTERNAL_ENDPOINT` (e.g. `http://minio:9000`, the Compose service DNS name) used for admin/bucket-management operations (create bucket, `HeadObject` checks, etc.) issued from inside the container, and `STORAGE_PUBLIC_ENDPOINT` (e.g. `http://localhost:9000`, the host-mapped port) used **only** when constructing the `S3Client` instance that signs presigned URLs — so the signature is computed against the externally-reachable host:port and is resolvable by a browser or a host-side script. In practice this means two `S3Client` instances (or one client whose endpoint is swapped per-call), both pointed at the same underlying MinIO server, differing only in which hostname is baked into their requests/signatures. **Which component needs which endpoint (resolving `AMB-4`):** only the API constructs the presigning `S3Client` and needs `STORAGE_PUBLIC_ENDPOINT` — it is the sole issuer of presigned URLs, for both TD-02's upload handshake and TD-08's streaming/download URLs. The worker (TD-05) never issues a presigned URL to anyone: it is not an HTTP-facing component and has no code path that hands a URL to an external client. Instead, the worker performs its own direct, server-side reads/writes against storage — downloading the original file to run `ffprobe`/`ffmpeg` against it, and uploading the generated thumbnail — using only `STORAGE_INTERNAL_ENDPOINT` via an admin-style `S3Client`, exactly like the API's own admin operations. Consequently, `STORAGE_PUBLIC_ENDPOINT` is a required env var for the API process only; the worker's Joi validation schema only needs to declare `STORAGE_INTERNAL_ENDPOINT` (plus the shared credentials/bucket name), not `STORAGE_PUBLIC_ENDPOINT`.
